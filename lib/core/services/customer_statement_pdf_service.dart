...
class CustomerStatementPdfService {
  ...
  static Future<Uint8List> generateStatementPdf({
    ...
    for (final SaleModel sale in customerSales) {
      ...
      sortedTransactions.add(
        LedgerTransactionModel(
          ...
          referenceId: sale.id,
          date: sale.date,
          notes: sale.invoiceNumber.trim().isEmpty
              ? 'Sale'
    ...
    for (final LedgerTransactionModel transaction in sortedTransactions) {
    ...
    final double outstanding = sortedTransactions.isEmpty
        ? 0
        : sortedTransactions.last.balanceAfter;

    final PdfColor primaryColor = PdfColor.fromHex('#2563EB');

    final PdfColor successColor = PdfColor.fromHex('#16A34A');

    final PdfColor dangerColor = PdfColor.fromHex('#DC2626');

    final PdfColor greyColor = PdfColor.fromHex('#64748B');

    final PdfColor lightGreyColor = PdfColor.fromHex('#F1F5F9');
              ...
              children: [
                ...
                pw.Expanded(
                  child: pw.Column(
                    ...
                    children: [
                      pw.Text(
                        _safeText(business.businessName, 'Business'),
                        style: pw.TextStyle(
                          fontSize: 22,
                  ...
                  pw.Expanded(
                    child: pw.Column(
                      ...
                      children: [
                        ...
                        pw.Text(
                          _safeText(customer.name, 'Customer'),
                          style: pw.TextStyle(
                            fontSize: 14,
                            fontWeight: pw.FontWeight.bold,
                          ),
                    ...
                    child: pw.Column(
                      ...
                      children: [
                        ...
                        if (customer.email.trim().isNotEmpty)
                          _infoRow('Email', customer.email.trim(), greyColor),
                        if (customer.gstNumber.trim().isNotEmpty)
                          _infoRow('GST', customer.gstNumber.trim(), greyColor),
                      ],
                    ),
                  ),
                ],
              ...
              children: [
                ...
                pw.Expanded(
                  child: _summaryBox(
                    ...
                    color: outstanding > 0 ? dangerColor : successColor,
                  ),
                ),
              ],
            ),
              ...
              pw.Table(
                ...
                children: [
                  ...
                  ...sortedTransactions.map((
                    ...
                    final double debit = isSale ? transaction.amount : 0;

                    final double credit = isPayment ? transaction.amount : 0;

                    return pw.TableRow(
                      children: [
                        _bodyCell(_formatDate(transaction.date)),
                        _bodyCell(_transactionLabel(type)),
                        _bodyCell(
  ...
  static Future<void> printStatement({
    ...
    final Uint8List pdfBytes = await generateStatementPdf(
      business: business,
      customer: customer,
  ...
  static Future<void> shareStatement({
    ...
    final String customerName = _sanitizeFileName(
      customer.name.trim().isEmpty ? 'Customer' : customer.name.trim(),
    );

    await Printing.sharePdf(
      bytes: pdfBytes,
      filename: 'Customer_Statement_$customerName.pdf',
    );
  }
  ...
  static pw.Widget _infoRow(String label, String value, PdfColor greyColor) {
    return pw.Padding(
  ...
  static pw.Widget _summaryBox({
    ...
    return pw.Container(
      ...
      decoration: pw.BoxDecoration(
        ...
        borderRadius: pw.BorderRadius.circular(7),
        border: pw.Border.all(color: color.shade(0.75), width: 0.7),
      ),
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          pw.Text(
            title,
            style: pw.TextStyle(fontSize: 7.5, color: color.shade(0.25)),
          ),
          pw.SizedBox(height: 4),
          pw.Text(
            value,
  ...
  static String _transactionLabel(String type) {
    switch (type) {
      ...
      default:
        return type.isEmpty ? 'Transaction' : _capitalize(type);
    }
  }
  ...
  static String _formatPeriod(DateTime? fromDate, DateTime? toDate) {
