...
class _SuppliersScreenState extends State<SuppliersScreen> {
  ...
  Future<void> _openAddSupplier() async {
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => AddSupplierScreen(businessId: businessId),
      ),
    );
  }

  Future<void> _openEditSupplier(SupplierModel supplier) async {
    final BusinessProvider businessProvider = context.read<BusinessProvider>();
    ...
    await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) =>
            AddSupplierScreen(businessId: businessId, supplier: supplier),
      ),
    );
  }
  ...
  Future<void> _deleteSupplier(SupplierModel supplier) async {
    ...
    try {
      await supplierProvider.deleteSupplier(supplierId: supplier.id);
      ...
      _showMessage(
        supplierProvider.errorMessage ?? 'Unable to delete supplier.',
        isError: true,
      );
    }
  }
  ...
  List<SupplierModel> _filterSuppliers(List<SupplierModel> suppliers) {
    ...
    return suppliers.where((supplier) {
      return supplier.name.toLowerCase().contains(_searchQuery) ||
          supplier.contactPerson.toLowerCase().contains(_searchQuery) ||
          supplier.mobile.toLowerCase().contains(_searchQuery) ||
          supplier.email.toLowerCase().contains(_searchQuery) ||
          supplier.address.toLowerCase().contains(_searchQuery) ||
          supplier.gstNumber.toLowerCase().contains(_searchQuery);
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (businessProvider.isLoading && businessProvider.business == null) {
      return Scaffold(
    ...
    final List<SupplierModel> allSuppliers = supplierProvider.suppliers;

    final List<SupplierModel> filteredSuppliers = _filterSuppliers(
      allSuppliers,
    );
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Refresh',
            onPressed: supplierProvider.isLoading ? null : _refresh,
  ...
  Widget _buildBody(
    ...
    if (provider.isLoading && allSuppliers.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    }

    if (provider.errorMessage != null && allSuppliers.isEmpty) {
      return _buildErrorState(context, theme, provider.errorMessage!);
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        padding: EdgeInsets.fromLTRB(
          MediaQuery.sizeOf(context).width < 420 ? 12 : 20,
          16,
          MediaQuery.sizeOf(context).width < 420 ? 12 : 20,
          110,
        ),
        children: [
          _buildHeader(theme, allSuppliers.length),
          ...
          if (allSuppliers.isEmpty)
            ...
            _buildSupplierList(context, theme, filteredSuppliers),
        ],
      ),
    );
  }

  Widget _buildHeader(ThemeData theme, int totalSuppliers) {
    return Container(
      padding: EdgeInsets.all(MediaQuery.sizeOf(context).width < 420 ? 14 : 20),
      ...
      child: Row(
        children: [
          Container(
            width: MediaQuery.sizeOf(context).width < 420 ? 48 : 56,
            height: MediaQuery.sizeOf(context).width < 420 ? 48 : 56,
            decoration: BoxDecoration(
            ...
            child: Icon(
              ...
              size: MediaQuery.sizeOf(context).width < 420 ? 25 : 30,
            ),
          ),
          SizedBox(width: MediaQuery.sizeOf(context).width < 420 ? 10 : 16),
          Expanded(
            child: Column(
          ...
          SizedBox(width: MediaQuery.sizeOf(context).width < 420 ? 6 : 10),
          Container(
            ...
            child: Column(
              children: [
                ...
                Text('Suppliers', style: theme.textTheme.labelSmall),
              ],
            ),
          ),
        ],
  ...
  Widget _buildSearchField(ThemeData theme) {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: 'Search by name, contact, mobile or GST...',
    ...
    return Column(
      children: [
        for (int index = 0; index < suppliers.length; index++) ...[
          _buildSupplierCard(context, theme, suppliers[index]),
          if (index != suppliers.length - 1) const SizedBox(height: 12),
        ],
      ],
    );
  }
  ...
  Widget _buildSupplierCard(
    ...
    final String initials = _getInitials(supplier.name);
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              _buildSupplierMenu(context, supplier),
            ],
          ),
        ),
      ),
  ...
  Widget _buildSupplierAvatar(String initials) {
    return Container(
      width: 52,
      height: 52,
      decoration: BoxDecoration(
  ...
  Widget _buildSupplierMenu(BuildContext context, SupplierModel supplier) {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    _buildSupplierAvatar(_getInitials(supplier.name)),
                    const SizedBox(width: 14),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
