...
class _SupplierStatementScreenState extends State<SupplierStatementScreen> {
  ...
  List<PurchaseModel> get _filteredPurchases {
    ...
    return _allSupplierPurchases.where((p) {
      ...
      final statusOk =
          ...
          (_filter == 'Pending' && status != 'paid');
      final dateOk =
          (start == null || !p.date.isBefore(start)) &&
          (end == null || !p.date.isAfter(end));
      return statusOk && dateOk;
    }).toList();
  }

  double _total(List<PurchaseModel> list) =>
      list.fold(0, (v, p) => v + p.total);
  double _paid(List<PurchaseModel> list) =>
      list.fold(0, (v, p) => v + p.paidAmount);
  double _pending(List<PurchaseModel> list) => list.fold(
    0,
    (v, p) => v + (p.total - p.paidAmount).clamp(0, double.infinity),
  );
  ...
  Future<Uint8List> _buildPdf() async {
    ...
    final businessName = business?.businessName.trim().isNotEmpty == true
        ? business!.businessName
        : 'Business';
    final period = _startDate == null
        ? 'All transactions'
        : '${_date(_startDate!)} to ${_date(_endDate!)}';
    doc.addPage(
      pw.MultiPage(
        ...
        footer: (context) => pw.Align(
          ...
          child: pw.Text(
            'Supplier Statement • Page ${context.pageNumber}',
        ...
        build: (context) => [
              ...
              pw.Expanded(
                child: pw.Column(
                  ...
                  children: [
                    ...
                    if (business?.mobile.trim().isNotEmpty == true)
                      pw.Text(business!.mobile),
          ...
          if (widget.supplier.mobile.trim().isNotEmpty)
            pw.Text('Mobile: ${widget.supplier.mobile}'),
          if (widget.supplier.gstNumber.trim().isNotEmpty)
            pw.Text('GST: ${widget.supplier.gstNumber}'),
            ...
            data: list
                .map(
                  (p) => [
                    ...
                    'Rs. ${(p.total - p.paidAmount).clamp(0, double.infinity).toStringAsFixed(2)}',
                    p.paymentStatus,
                  ],
                )
                .toList(),
            ...
            (p) => pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  ...p.items.map(
                    (i) => pw.Text(
                      '  ${i.productName} • ${i.quantity} ${i.unit} • Rs. ${i.total.toStringAsFixed(2)}',
                    ),
                  ),
                ],
              ),
  ...
  Future<void> _sharePdf() async {
    ...
    try {
      ...
      final file = XFile.fromData(
        bytes,
        name:
            'supplier_statement_${widget.supplier.name.replaceAll(RegExp(r'[^a-zA-Z0-9]+'), '_')}.pdf',
        mimeType: 'application/pdf',
      );
      await SharePlus.instance.share(
        ShareParams(
          files: [file],
          text: 'Supplier statement for ${widget.supplier.name}.',
        ),
      );
    } catch (e) {
  ...
  Future<void> _shareImage() async {
    ...
    try {
      ...
      final file = XFile.fromData(
        imageBytes,
        name:
            'supplier_statement_${widget.supplier.name.replaceAll(RegExp(r'[^a-zA-Z0-9]+'), '_')}.png',
        mimeType: 'image/png',
      );
      await SharePlus.instance.share(
        ShareParams(
          files: [file],
          text: 'Supplier statement image for ${widget.supplier.name}.',
        ),
      );
    } catch (e) {
          ...
          appBar: AppBar(
            ...
            actions: [
              IconButton(
                ...
                onPressed: list.isEmpty ? null : _downloadPdf,
                icon: const Icon(Icons.picture_as_pdf_outlined),
              ),
              IconButton(
                tooltip: 'Share PDF',
                onPressed: list.isEmpty || _sharing ? null : _sharePdf,
          ...
          body: AppResponsivePage(child: RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                if (widget.supplier.mobile.isNotEmpty)
                  Text(widget.supplier.mobile),
                ...
                SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      for (final value in ['All', 'Paid', 'Pending'])
                        Padding(
                ...
                Row(
                  children: [
                    ...
                    OutlinedButton.icon(
                      onPressed: list.isEmpty || _sharing ? null : _shareImage,
                      ...
                      children: [
                        ...p.items.map(
                          (i) => ListTile(
                            ...
                            subtitle: Text(
                              '${i.quantity} ${i.unit} × ${_money(i.purchaseRate)}',
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                            ),
                            trailing: ConstrainedBox(
                        ...
                        Align(
                          ...
                          child: ConstrainedBox(
                            ...
                            child: Text(
                              'Paid: ${_money(p.paidAmount)} • Pending: ${_money((p.total - p.paidAmount).clamp(0, double.infinity))}',
                              maxLines: 2,
                              overflow: TextOverflow.ellipsis,
                              textAlign: TextAlign.end,
