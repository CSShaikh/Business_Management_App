...
class _SupplierReportScreenState extends State<SupplierReportScreen> {
  ...
  Future<void> _loadReport({bool showLoader = true}) async {
    ...
    try {
      ...
      final results = await Future.wait([
        _supplierRepository.getSuppliers(business.id),
        _purchaseRepository.getPurchases(businessId: business.id),
        _supplierPaymentRepository.getPayments(businessId: business.id),
      ]);
      ...
      setState(() {
        ...
        _suppliers = results[0] as List<SupplierModel>;

        _purchases = results[1] as List<PurchaseModel>;

        _supplierPayments = results[2] as List<SupplierPaymentModel>;
  ...
  List<SupplierPaymentModel> _supplierPaymentsFor(SupplierModel supplier) {
    return _filteredSupplierPayments.where((payment) {
      return payment.supplierId.trim() == supplier.id.trim();
    }).toList();
  }
  ...
  List<SupplierModel> get _filteredSuppliers {
    ...
    final Set<String> supplierIds = <String>{
      ..._filteredPurchases
          .map((purchase) => purchase.supplierId.trim())
          .where((id) => id.isNotEmpty),
      ..._filteredSupplierPayments
          .map((payment) => payment.supplierId.trim())
          .where((id) => id.isNotEmpty),
    };

    final List<SupplierModel> suppliers = _suppliers.where((supplier) {
      final bool matchesSearch =
          ...
          supplier.gstNumber.toLowerCase().contains(query);
      ...
      return supplierIds.contains(supplier.id);
    }).toList();
  ...
  List<PurchaseModel> _supplierPurchases(SupplierModel supplier) {
    return _filteredPurchases.where((purchase) {
      return purchase.supplierId.trim() == supplier.id.trim();
    }).toList();
  }

  double _supplierPurchaseAmount(SupplierModel supplier) {
    return _supplierPurchases(supplier)
        .fold(0, (sum, purchase) => sum + purchase.total);
  }

  double _supplierPurchasePaidAmount(SupplierModel supplier) {
    return _supplierPurchases(supplier)
        .fold(0, (sum, purchase) => sum + purchase.paidAmount);
  }

  double _supplierPaymentAmount(SupplierModel supplier) {
    return _supplierPaymentsFor(supplier)
        .fold(0, (sum, payment) => sum + payment.amount);
  }

  double _supplierPaidAmount(SupplierModel supplier) {
    return _supplierPurchasePaidAmount(supplier) +
        _supplierPaymentAmount(supplier);
  }

  double _supplierOutstandingAmount(SupplierModel supplier) {
    final double outstanding =
        _supplierPurchaseAmount(supplier) - _supplierPaidAmount(supplier);

    return outstanding > 0 ? outstanding : 0;
  }

  double _supplierQuantity(SupplierModel supplier) {
    return _supplierPurchases(supplier).fold(0, (sum, purchase) {
      return sum +
          purchase.items.fold(0, (itemSum, item) => itemSum + item.quantity);
    });
  }

  int _supplierPurchaseCount(SupplierModel supplier) {
  ...
  double get _totalPurchases {
    return _filteredPurchases.fold(0, (sum, purchase) => sum + purchase.total);
  }
  ...
  double get _totalPaid {
    return _totalPurchasePaid + _totalSupplierPayments;
  }

  double get _totalOutstanding {
    final double outstanding = _totalPurchases - _totalPaid;
  ...
  int get _paidPurchaseCount {
    return _filteredPurchases
        .where((purchase) => purchase.paymentStatus.toLowerCase() == 'paid')
        .length;
  }

  int get _partialPurchaseCount {
    return _filteredPurchases
        .where((purchase) => purchase.paymentStatus.toLowerCase() == 'partial')
        .length;
  }

  int get _unpaidPurchaseCount {
    return _filteredPurchases
        .where((purchase) => purchase.paymentStatus.toLowerCase() == 'unpaid')
        .length;
  }
  ...
  double get _paymentRate {
    ...
    return (_totalPaid / _totalPurchases) * 100;
  }
  ...
  Future<void> _downloadPdf() async {
    ...
    try {
        ...
        pw.MultiPage(
          ...
          build: (context) => [
            ...
            if (_filteredSupplierPayments.isEmpty)
              pw.Text(
                'No supplier payment records found for the selected period.',
              )
            else
              pw.TableHelper.fromTextArray(
                headers: const [
      ...
      if (!mounted) return;
      _showMessage('PDF download failed: ${e.toString()}', isError: true);
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget _buildHeader(ThemeData theme, bool isDesktop) {
    final String dateText = _startDate != null && _endDate != null
        ? '${_date(_startDate!)} - ${_date(_endDate!)}'
        : 'All dates';
  ...
  Widget _buildSummary(bool isDesktop) {
    final List<_SummaryItem> items = [
      ...
      _SummaryItem(
        ...
        subtitle: 'Purchase + supplier payments',
      ...
      _SummaryItem(
        title: 'Suppliers',
        value: '${_filteredSuppliers.length}',
        subtitle: 'Suppliers in report',
      ...
      child: Column(
        ...
        children: [
          const _SectionHeader(
            ...
            subtitle: 'Purchase payment status and supplier payment activity',
          ),
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              final List<Widget> items = [
                _PaymentStatusTile(
                  title: 'Paid',
                  count: _paidPurchaseCount,
                  color: AppColors.success,
                ),
                _PaymentStatusTile(
                  title: 'Partial',
                  count: _partialPurchaseCount,
                  color: AppColors.warning,
                ),
                _PaymentStatusTile(
                  title: 'Unpaid',
                  count: _unpaidPurchaseCount,
              ...
              Expanded(
                child: ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    value: (_paymentRate / 100).clamp(0.0, 1.0),
                    minHeight: 8,
            ...
            children: [
              Expanded(
                child: _MiniMetric(
                  ...
                  value: _currency(_totalPurchasePaid),
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Search supplier, contact, mobile, email or GST...',
  ...
  Widget _buildSupplierList(ThemeData theme, List<SupplierModel> suppliers) {
    return _ReportCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _SectionHeader(
            icon: Icons.local_shipping_rounded,
            title: 'Supplier-wise Performance',
            subtitle:
          ...
          if (suppliers.isEmpty)
            const _EmptyInline(
              ...
              message: 'No suppliers match the selected filters.',
            )
          else
            ...suppliers.map((supplier) {
              return _SupplierTile(
                supplier: supplier,
                purchaseCount: _supplierPurchaseCount(supplier),
                quantity: _supplierQuantity(supplier),
                totalPurchases: _supplierPurchaseAmount(supplier),
                paid: _supplierPaidAmount(supplier),
                outstanding: _supplierOutstandingAmount(supplier),
                currency: _currency,
                number: _number,
                onTap: () {
  ...
  void _showSupplierDetails(SupplierModel supplier) {
    final List<PurchaseModel> purchases = _supplierPurchases(supplier);

    final List<SupplierPaymentModel> supplierPayments = _supplierPaymentsFor(
      supplier,
    );

    final double total = _supplierPurchaseAmount(supplier);

    final double paid = _supplierPaidAmount(supplier);

    final double separatePayments = _supplierPaymentAmount(supplier);

    final double outstanding = _supplierOutstandingAmount(supplier);
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  _MiniMetric(
                    label: 'Separate Supplier Payments',
                  ...
                  if (supplier.mobile.trim().isNotEmpty)
                    _DetailRow(label: 'Mobile', value: supplier.mobile),
                  ...
                  if (supplier.gstNumber.trim().isNotEmpty)
                    _DetailRow(label: 'GST Number', value: supplier.gstNumber),
                  const SizedBox(height: 12),
                  Text(
                    'Recent Purchases',
                    style: theme.textTheme.titleMedium?.copyWith(
                  ...
                  if (purchases.isEmpty)
                    const _EmptyInline(
                      ...
                      message: 'No purchases found for this supplier in the selected period.',
                    )
                  else
                    ...purchases.reversed.take(10).map((purchase) {
                      final double due = purchase.total - purchase.paidAmount;
                  ...
                  if (supplierPayments.isEmpty)
                    ...
                    ...supplierPayments.toList().reversed.take(10).map((
                      payment,
                    ) {
                      return Container(
                        margin: const EdgeInsets.only(bottom: 8),
...
class _SupplierDetailSummary extends StatelessWidget {
  final String total;
  final String paid;
