...
class _CustomerReportScreenState extends State<CustomerReportScreen> {
  ...
  Future<void> _loadReport({bool showLoader = true}) async {
    ...
    try {
      ...
      final results = await Future.wait([
        _customerRepository.getCustomers(businessId: business.id),
  ...
  List<PaymentModel> _customerPayments(CustomerModel customer) {
    return _filteredPayments.where((payment) {
      return payment.customerId.trim() == customer.id.trim();
    }).toList();
  }
  ...
  List<CustomerModel> get _filteredCustomers {
    ...
    final Set<String> customerIds = <String>{
      ..._filteredSales
          .map((sale) => sale.customerId.trim())
          .where((id) => id.isNotEmpty),
      ..._filteredPayments
          .map((payment) => payment.customerId.trim())
          .where((id) => id.isNotEmpty),
    };

    final List<CustomerModel> customers = _customers.where((customer) {
  ...
  List<SaleModel> _customerSales(CustomerModel customer) {
    return _filteredSales.where((sale) {
      return sale.customerId.trim() == customer.id.trim();
    }).toList();
  }

  double _customerSalesAmount(CustomerModel customer) {
    return _customerSales(customer).fold(0, (sum, sale) => sum + sale.total);
  }
  ...
  double _customerReceivedAmount(CustomerModel customer) {
    final double salePayments = _customerSales(customer)
        .fold<double>(0, (sum, sale) => sum + sale.paidAmount);

    final double separatePayments = _customerPayments(customer)
  ...
  double _customerOutstandingAmount(CustomerModel customer) {
    final double outstanding =
        _customerSalesAmount(customer) - _customerReceivedAmount(customer);
  ...
  double _customerProfit(CustomerModel customer) {
    return _customerSales(customer).fold(0, (sum, sale) {
      final double saleProfit = sale.items.fold(0, (itemSum, item) {
        return itemSum + ((item.sellingRate - item.costPrice) * item.quantity);
      });
  ...
  int _customerInvoiceCount(CustomerModel customer) {
  ...
  double get _totalReceived {
    final double salePayments = _filteredSales.fold<double>(
      0,
      (sum, sale) => sum + sale.paidAmount,
    );

    final double separatePayments = _filteredPayments.fold<double>(
      0,
  ...
  double get _totalOutstanding {
    final double outstanding = _totalSales - _totalReceived;

    return outstanding > 0 ? outstanding : 0;
  }

  double get _totalProfit {
    return _filteredSales.fold(0, (sum, sale) {
      final double profit = sale.items.fold(0, (itemSum, item) {
        return itemSum + ((item.sellingRate - item.costPrice) * item.quantity);
      });
  ...
  double get _collectionRate {
    ...
    return (_totalReceived / _totalSales) * 100;
  }
                ...
                child: ConstrainedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildCustomerList(theme, customers),
                    ],
                  ),
                ),
              ),
  ...
  Widget _buildSummary(bool isDesktop) {
    final List<_SummaryItem> items = [
      _SummaryItem(
        ...
        subtitle: 'Sales in selected period',
      ...
      _SummaryItem(
        title: 'Received',
        value: _currency(_totalReceived),
        subtitle: 'Sale payments + customer payments',
      ...
      _SummaryItem(
        title: 'Customers',
        value: _number(_activeCustomerCount.toDouble()),
        subtitle: 'Customers in report',
      ...
      child: Column(
        ...
        children: [
          const _SectionHeader(
            ...
            subtitle: 'Collection and profitability overview',
          ),
            ...
            builder: (context, constraints) {
              ...
              final List<Widget> cards = [
                _HealthTile(
                  ...
                  subtitle: 'Sales already received',
                ...
                _HealthTile(
                  ...
                  subtitle: 'Separate customer payments',
                  icon: Icons.payments_rounded,
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Search customer, hotel, mobile, email or GST...',
  ...
  Widget _buildCustomerList(ThemeData theme, List<CustomerModel> customers) {
    return _ReportCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _SectionHeader(
            icon: Icons.groups_rounded,
            title: 'Customer-wise Performance',
            subtitle:
          ...
          if (customers.isEmpty)
            ...
            ...customers.map((customer) {
              return _CustomerTile(
                customer: customer,
                sales: _customerSales(customer),
                totalSales: _customerSalesAmount(customer),
                received: _customerReceivedAmount(customer),
                outstanding: _customerOutstandingAmount(customer),
                profit: _customerProfit(customer),
                invoiceCount: _customerInvoiceCount(customer),
                currency: _currency,
                onTap: () {
  ...
  void _showCustomerDetails(CustomerModel customer) {
    final List<SaleModel> sales = _customerSales(customer);

    final List<PaymentModel> payments = _customerPayments(customer);

    final double salesAmount = _customerSalesAmount(customer);

    final double received = _customerReceivedAmount(customer);

    final double outstanding = _customerOutstandingAmount(customer);

    final double profit = _customerProfit(customer);
              ...
              child: Column(
                ...
                children: [
                  ...
                  _CustomerDetailSummary(
                    sales: _currency(salesAmount),
                    received: _currency(received),
                    outstanding: _currency(outstanding),
                    profit: _currency(profit),
                  ),

                  const SizedBox(height: 20),

                  _DetailRow(label: 'Owner / Contact', value: customer.name),

                  if (customer.mobile.trim().isNotEmpty)
                    _DetailRow(label: 'Mobile', value: customer.mobile),
                  ...
                  if (customer.gstNumber.trim().isNotEmpty)
                    _DetailRow(label: 'GST Number', value: customer.gstNumber),
                  ...
                  if (sales.isEmpty)
                    ...
                    ...sales.reversed.take(10).map((sale) {
                      final double due = sale.total - sale.paidAmount;
                  ...
                  if (payments.isEmpty)
                    const _EmptyInline(
                      ...
                      message: 'No separate customer payments found in the selected period.',
                    )
                  else
                    ...payments.reversed.take(10).map((payment) {
                      return Container(
                        margin: const EdgeInsets.only(bottom: 8),
                        padding: const EdgeInsets.all(13),
                        decoration: BoxDecoration(
...
class _CustomerDetailSummary extends StatelessWidget {
  final String sales;
  ...
  Widget build(BuildContext context) {
    final List<_DetailMetric> items = [
      _DetailMetric(label: 'Sales', value: sales, color: AppColors.primary),
      ...
      _DetailMetric(label: 'Profit', value: profit, color: AppColors.secondary),
    ];
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        final _DetailMetric item = items[index];
