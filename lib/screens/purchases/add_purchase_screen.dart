...
class _AddPurchaseScreenState extends State<AddPurchaseScreen> {
  ...
  Future<void> _initialize() async {
    try {
      ...
      if (business == null) {
        setState(() {
          _isLoading = false;
          _errorMessage = 'Business information was not found.';
        });
        return;
      }
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = 'Unable to load purchase information.';
      });
    }
  }

  Future<void> _loadExistingPurchase(PurchaseModel purchase) async {
    ...
    if (purchase.supplierId.isNotEmpty && _business != null) {
      try {
        _selectedSupplier = await _supplierRepository.getSupplier(
          _business!.id,
          purchase.supplierId,
        );
      } catch (_) {
    ...
    final products = _business == null
        ? <ProductModel>[]
        : await _productRepository.getProducts(_business!.id);
  ...
  double get _tax {
    final value = _parseDouble(_taxController.text);
  ...
  double get _paidAmount {
    final value = _parseDouble(_paidAmountController.text);
  ...
  Future<void> _selectSupplier() async {
    ...
    final suppliers = await _supplierRepository.getSuppliers(_business!.id);
  ...
  Future<void> _addProduct() async {
    ...
    final products = await _productRepository.getActiveProducts(_business!.id);
    ...
    final selected = await showModalBottomSheet<ProductModel>(
      ...
      showDragHandle: true,
      builder: (context) {
        return _ProductPickerSheet(products: products);
      },
    );
    ...
    if (existingIndex >= 0) {
      ...
      _showMessage('${selected.name} quantity increased.');
      return;
    }
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () {
                ...
                final rate = _parseDouble(rateController.text);

                if (quantity <= 0 || rate < 0) {
                  return;
                }

                Navigator.pop(context, {'quantity': quantity, 'rate': rate});
              },
  ...
  Future<void> _savePurchase() async {
    ...
    if (_total <= 0) {
      _showMessage('Purchase total must be greater than zero.', isError: true);
      return;
    }
    ...
    try {
      ...
      _showMessage('Unable to save purchase: $e', isError: true);
    }
  }
    ...
    return Column(
      ...
      children: [
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Expanded(flex: 3, child: _buildSupplierCard()),
            const SizedBox(width: 16),
            Expanded(flex: 2, child: _buildDateAndPaymentCard()),
          ],
        ),
        const SizedBox(height: 16),
        _buildProductsCard(),
        ...
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Expanded(flex: 3, child: _buildNotesCard()),
            const SizedBox(width: 16),
        ...
        child: Column(
          ...
          children: [
            const _SectionTitle(
              ...
              title: 'Supplier',
              subtitle: 'Select the supplier for this purchase',
            ),
            const SizedBox(height: 16),
            InkWell(
        ...
        child: Column(
          ...
          children: [
            const _SectionTitle(
              ...
              subtitle: 'Date and payment information',
            ),
            const SizedBox(height: 16),
            InkWell(
              onTap: _selectDate,
        ...
        child: Column(
          ...
          children: [
            const _SectionTitle(
              ...
              subtitle: 'Review the final purchase amount',
            ),
            const SizedBox(height: 16),
            ...
            _AmountRow(label: 'Outstanding', value: _outstanding, isBold: true),
          ],
        ),
      ),
    );
        ...
        child: Column(
          ...
          children: [
            const _SectionTitle(
              ...
              title: 'Notes',
              subtitle: 'Optional notes for this purchase',
            ),
            const SizedBox(height: 16),
            TextFormField(
...
class _PurchaseItemCardState extends State<_PurchaseItemCard> {
  ...
  void _updateQuantity(String value) {
    final double quantity = _parseDouble(value);
  ...
  void _updateRate(String value) {
    final double rate = _parseDouble(value);
    widget.item.purchaseRate = rate;
    widget.onChanged();
    setState(() {});
  }
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              final Widget quantityField = TextFormField(
                ...
                decoration: _fieldDecoration(label: 'Quantity', prefix: ''),
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            subtitle: supplier.mobile.trim().isEmpty
                                ? null
