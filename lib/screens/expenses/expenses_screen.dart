...
class _ExpensesScreenState extends State<ExpensesScreen> {
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      expenseProvider.setBusinessId(businessId);

      await expenseProvider.loadAndWatchExpenses(businessId: businessId);
      ...
      setState(() {
        _isInitializing = false;
        _initialized = false;
        _pageError = 'Unable to load expenses. Please try again.';
      });
    }
  }
  ...
  Future<void> _refresh() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Unable to refresh expenses.'),
  ...
  List<ExpenseModel> _filterExpenses(List<ExpenseModel> expenses) {
    ...
    return expenses.where((expense) {
      if (query.isNotEmpty) {
        final String category = expense.category.toLowerCase();

        final String paymentMethod = expense.paymentMethod.toLowerCase();

        final String description = expense.description.toLowerCase();

        final String notes = expense.notes.toLowerCase();

        final String amount = expense.amount.toStringAsFixed(2);
  ...
  double _calculateTotal(List<ExpenseModel> expenses) {
    return expenses.fold<double>(0, (double total, ExpenseModel expense) {
      return total + expense.amount;
    });
  }

  double _calculateTodayTotal(List<ExpenseModel> expenses) {
  ...
  Future<void> _selectDateRange() async {
    ...
    // Use the app-level calendar instead of Flutter's native date-range
    // picker. The app picker is calendar-only: no text-input mode and no
    // internal scrolling. From/To are selected directly on the calendar.
          ...
          child: Container(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  _buildDetailsHeader(context, expense),
                  ...
                  _detailTile(
                    ...
                    label: 'Amount',
                    value: _currencyFormat.format(expense.amount),
  ...
  Widget _buildDetailsHeader(BuildContext context, ExpenseModel expense) {
    return Row(
      children: [
        Container(
          width: 50,
      ...
      builder:
          (
            ...
            if (expenseProvider.errorMessage != null &&
              ...
              return _buildProviderError(expenseProvider.errorMessage!);
            }

            final List<ExpenseModel> allExpenses = expenseProvider.expenses;

            final List<ExpenseModel> filteredExpenses = _filterExpenses(
              allExpenses,
            );

            final double totalExpenses = _calculateTotal(allExpenses);

            final double filteredTotal = _calculateTotal(filteredExpenses);

            final double todayTotal = _calculateTodayTotal(allExpenses);
  ...
  Widget _buildHeader(int expenseCount, double todayTotal) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
  ...
  Widget _searchField() {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: 'Search category, description, amount...',
  ...
  Widget _buildExpenseList(List<ExpenseModel> expenses) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        ...expenses.map((ExpenseModel expense) => _expenseCard(expense)),
      ],
    );
  }
  ...
  Widget _expenseCard(ExpenseModel expense) {
    ...
    final String description = expense.description.trim().isEmpty
        ? 'No description'
        : expense.description;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      runSpacing: 5,
                      children: [
                        _smallTag(category, AppColors.danger),
                        Text(
                          _dateFormat.format(expense.date),
                          style: Theme.of(context).textTheme.bodySmall,
                        ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            _hasFilters ? 'No expenses found' : 'No expenses recorded',
            style: Theme.of(context).textTheme.titleMedium
                ?.copyWith(fontWeight: FontWeight.w700),
          ),
